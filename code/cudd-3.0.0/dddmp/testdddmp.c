...
static int MintermCount(DdManager *ddMgr, DdNode **operandBdd, dddmpVarInfo_t *varInfo);
  ...
  )
{
 ...
 rootmatchmode = DDDMP_ROOT_MATCHLIST;
  ...
  while (1) {
    ...
    } else if (strncmp (row, "mc", 2)==0) {
      MintermCount (ddMgr, operandBdd, &varInfo);
  ...
  )
{
  ...
  fprintf (stdout, "\tmc   : Minterm Count.\n");
...
/**Function********************************************************************
  ...
  Synopsis     [Count the number of minterms of a BDD.]
  
  Description  [Count the number of minterms of a BDD.]
  ...
  )
{
  ...
  retValue = Cudd_CountMinterm (ddMgr, f, varInfo->nDdVars);

  Dddmp_CheckAndGotoLabel (retValue==CUDD_OUT_OF_MEM,
    "Minterm Not Counted.", failure);

  fprintf (stdout, "Minterm Count: %g\n", retValue);
  fflush (stdout);

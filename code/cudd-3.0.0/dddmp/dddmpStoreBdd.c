...
)
{
    ...
    if (varnames != NULL) {
        // DFA 29-01-2019 Write ALL variable names
        retValue = fprintf (fp, ".varnames");
        Dddmp_CheckAndGotoLabel (retValue==EOF, "Error writing to file.",
                                 failure);
        ...
        for(i=0; i<nVars;i++) {
            if (varnames[i] == NULL) {
                ...
                fflush (stderr);
                varnames[i] = DDDMP_ALLOC (char, 10);
                Dddmp_CheckAndGotoLabel (varnames[i] == NULL,
                                         "Error allocating memory.", failure);
                sprintf (varnames[i], "DUMMY%d", i);
            }
            retValue = fprintf (fp, " %s", varnames[i]);
            Dddmp_CheckAndGotoLabel (retValue==EOF, "Error writing to file.",
                                     failure);
        }
        retValue = fprintf (fp, "\n");
